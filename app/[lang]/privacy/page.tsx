...
export async function generateMetadata({ params }: { params: { lang: string } }) {
  ...
  return {
    title: `${dict.privacy.title} | Fresco`,
  }
}
    ...
    <div className="min-h-screen bg-[#FFF9F1] dark:bg-[#1D1D1D]">
      <div className="container px-4 py-8 md:py-12">
        ...
        <div className="max-w-3xl mx-auto">
          <h1 className="text-3xl font-bold mb-6 text-[#FCA336]">{dict.privacy.title}</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-8">
            {dict.privacy.lastUpdated} {currentDate}
          </p>

          <div className="space-y-8">
            {dict.privacy.content.map((section: any, index: number) => (
